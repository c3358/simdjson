...
public:
   ...
   *
   * **Performance Notes:** This is a lot less performant (up to 40% slower in informal tests) than
   * find_field(). It starts out normally, starting out at the last field; but if the field is not
   * found, it scans from the beginning of the object to see if it missed it. That missing case has
   * a non-cache-friendly bump and lots of extra scanning, especially if the object in question is
   * large.
   * 
